...
with open(jd_file_path, "rb") as f:
    files.append(
        (
            ...
            (os.path.basename(jd_file_path), f.read(), "text/plain"),
        )
    )
...
if response.status_code == 200:
    ...
    # Gemini 점수로 재정렬
    sorted_by_gemini = sorted(
        result["top_candidates"], key=lambda x: x.get("gemini_score", 0), reverse=True
    )
    ...
    print(
        f"   Comparing: {result['top_candidates'][0]['name']} vs {result['top_candidates'][1]['name']}"
    )
